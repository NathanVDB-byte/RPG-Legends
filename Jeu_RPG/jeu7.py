...
pygame.display.set_caption("Mini RPG - Timer & Fuite")
...
PURPLE = (128, 0, 128)
DARK_BLUE = (20, 20, 50)
ORANGE = (255, 140, 0) # Pour le bouton fuir
...
class Button:
    def __init__(self, text, x, y, w, h, color, data=None):
        ...
        self.text = text
        self.color = color
        self.original_color = color # Pour se souvenir de la couleur de base
        self.data = data
        self.hover = False
        self.disabled = False # Nouvel attribut pour le cooldown
...
class Game:
    def __init__(self):
        ...
        # Gestion du cooldown de repos
        self.next_rest_time = 0 # Timestamp (ms) à partir duquel on peut se reposer
    ...
    def setup_ui(self):
        ...
        # Combat (NOUVEAU: Boutons plus petits pour faire de la place à "Fuir")
        self.btn_attack = Button("ATTAQUER", 200, 450, 200, 60, RED)
        self.btn_flee = Button("FUIR 🏃", 420, 450, 180, 60, ORANGE) # Bouton Fuir
        ...
        while True:
            ...
            # =================== CAMP (Avec Timer Repos) ===================
                ...
                if time_left > 0:
                    ...
                    self.btn_rest.text = f"Repos ({seconds}s)"
                ...
                for event in events:
                    ...
                    if self.btn_rest.is_clicked(event):
                        ...
                        # Fixer le prochain repos dans 60 secondes (60000 ms)
            ...
            # =================== COMBAT (Avec Fuite) ===================
                ...
                self.btn_flee.check_hover(pos); self.btn_flee.draw(screen) # Bouton Fuir
                self.draw_logs()
                ...
                for event in events:
                    ...
                    if self.btn_flee.is_clicked(event):
                        if random.random() < 0.5: # 50% de chance
                            self.add_log("🏃 Vous avez fui le combat !")
                            self.state = "CAMP"
                        else:
                            self.add_log("⛔ Fuite ratée ! L'ennemi attaque.")
                            # Attaque gratuite de l'ennemi
                            dmg = max(0, self.enemy.attack_value - self.player.defense_value)
                            self.player.hp -= dmg
