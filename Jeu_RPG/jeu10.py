...
# Configuration des Étages (Monstres normaux)
FLOOR_ENEMIES = {
...
# Configuration des Boss par étage
FLOOR_BOSSES = {
...
pygame.display.set_caption("Mini RPG - RPG Legends")
...
class Character:
    def __init__(self, name, hp, attack, defense):
        ...
        self.inventory = []
        # NOUVELLES STATS DE PROGRESSION
        self.floor = 1
        self.kills = 0
    ...
    def from_dict(cls, data):
        ...
        c.floor = data.get("floor", 1) # Valeur par défaut 1
        c.kills = data.get("kills", 0) # Valeur par défaut 0
        return c
...
class Button:
    ...
    def draw(self, surface):
        draw_col = GRAY if self.disabled else (min(self.color[0]+30, 255), min(self.color[1]+30, 255), min(self.color[2]+30, 255)) if self.hover else self.color
...
class Game:
    ...
    # --- NOUVELLE FONCTION SPAWN ---
    def spawn_enemy(self):
        """Génère un ennemi ou un boss selon la progression."""
        floor = self.player.floor
        kills = self.player.kills

        # BOSS tous les 10 kills
        if kills >= 10:
            ...
            # Ennemi Normal adapté à l'étage
            # Si étage > 5, on garde les ennemis de l'étage 5 mais boostés
            ...
            # Petit facteur aléatoire pour varier les stats (+/- 20%)
            factor = random.uniform(0.8, 1.2)
            
            # Boost si on est dans les niveaux infinis (>5)
    ...
    def run(self):
        while True:
            ...
            if self.state == "MENU":
                ...
                for event in events:
                    if self.btn_new.is_clicked(event): self.state = "INPUT_NAME"; self.input_text = ""
            ...
            # CAMP
                ...
                for event in events:
                    if self.btn_explore.is_clicked(event):
                        self.spawn_enemy() # Appel de la nouvelle logique
                        self.state = "COMBAT"
                ...
                for event in events:
                    if self.btn_attack.is_clicked(event):
                        ...
                        if not self.enemy.is_alive():
                            ...
                            # Loot normal
                            if random.random() < 0.6: # 60% chance de loot
