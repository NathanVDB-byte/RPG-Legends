...
pygame.display.set_caption("Mini RPG - Final Corrected")
...
class Button:
    ...
    def is_clicked(self, event, offset_y=0, check_bounds=False):
        """
        check_bounds=True : active la protection anti-clic hors zone (pour les items scrollables).
        check_bounds=False : comportement normal (pour les boutons fixes).
        """
        adjusted_rect = self.rect.move(0, offset_y)
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            if adjusted_rect.collidepoint(event.pos):
                # Seulement si on demande explicitement de vérifier les limites (Items)
                if check_bounds:
                    if adjusted_rect.bottom < 100 or adjusted_rect.top > 520:
                        return False
                return True
        return False
...
class Game:
    ...
    def refresh_inventory_ui(self):
        ...
        if self.player:
            ...
            col_width = 240; row_height = 100; cols = 3
    ...
    def run(self):
        while True:
            ...
            for event in events:
                ...
                # Bouton Menu Global (Utilise check_bounds=False par défaut)
            ...
            if self.state == "MENU":
                ...
                self.draw_text_centered("INVENTAIRE (SCROLL)", FONT_TITLE, 50, PURPLE)
                clip_rect = pygame.Rect(0, 100, SCREEN_WIDTH, 420)
                screen.set_clip(clip_rect)
                
                for btn in self.item_buttons:
                    btn.check_hover(pos, self.inv_scroll_y)
                    btn.draw(screen, self.inv_scroll_y)
                    if btn.hover:
                ...
                for event in events:
                    ...
                    for btn in self.item_buttons:
                        # ICI: ON ACTIVE check_bounds=True SEULEMENT POUR LES ITEMS
                        ...
                        if not self.enemy.is_alive():
                            if self.enemy.is_boss:
                                ...
                                for loot in BOSS_LOOT:
                                    ...
                                    self.add_log(f"Récompense Boss: {loot['name']}")
                            else:
                                self.player.kills += 1
