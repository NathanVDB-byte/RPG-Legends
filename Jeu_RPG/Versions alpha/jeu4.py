...
pygame.display.set_caption("Mini RPG - Gestionnaire de Persos")
...
class Game:
    ...
    def refresh_save_list(self):
        ...
        if os.path.exists(SAVES_DIR):
            ...
            # Pagination simple : on affiche les 6 premiers
    ...
    def run(self):
        while True:
            ...
            for event in events:
                ...
                # Le bouton Menu Principal est dispo presque partout
                if self.state in ["CAMP", "STATS_VIEW"]:
            ...
            # =================== MENU CHARGEMENT ===================
                ...
                if not self.save_files_buttons:
                    self.draw_text_centered("Aucune sauvegarde.", FONT_TEXT, 250, LIGHT_GRAY)
                ...
                for event in events:
                    if self.btn_back_load.is_clicked(event):
                        self.state = "MENU"
                    ...
                    for btn in self.save_files_buttons:
                        if btn.is_clicked(event):
                            ...
                            try:
                                ...
                                # CHANGEMENT : On ne va pas au CAMP, mais aux STATS
                                self.state = "STATS_VIEW"
                            except Exception as e:
                                print(f"Erreur: {e}")

            # =================== VUE STATISTIQUES (NOUVEAU) ===================
            elif self.state == "STATS_VIEW":
                # Fond style "Carte de personnage"
                ...
                # Stats avec barres visuelles (optionnel mais joli)
                screen.blit(FONT_TEXT.render(f"❤️ Santé : {p.hp}/{p.max_hp}", True, WHITE), (250, 260))
                pygame.draw.rect(screen, RED, (450, 265, 100 * (p.hp/p.max_hp), 15)) # Barre de vie
                ...
                self.btn_menu.check_hover(pos); self.btn_menu.draw(screen) # Retour Menu Principal

                for event in events:
                    if self.btn_resume.is_clicked(event):
                        self.state = "CAMP"
                        self.add_log(f"Retour au camp avec {p.name}.")
                    if self.btn_back_stats.is_clicked(event):
                        self.state = "LOAD_MENU" # Retour à la liste des fichiers

            # =================== CRÉATION NOM ===================
            elif self.state == "INPUT_NAME":
                self.draw_text_centered("Nommez votre nouveau Héros :", FONT_TITLE, 150)
                ...
                for event in events:
                    ...
                    if self.btn_confirm_name.is_clicked(event) and self.input_text:
                        ...
                        self.state = "CAMP"
                        self.add_log(f"Aventure commencée !")
                        # Auto-save à la création pour qu'il apparaisse direct dans la liste
            ...
            # =================== CAMPEMENT ===================
                ...
                stats = f"{self.player.name} (PV: {self.player.hp}/{self.player.max_hp})"
                self.draw_text_centered(stats, FONT_SUBTITLE, 140, GREEN)
                ...
                for event in events:
                    if self.btn_attack.is_clicked(event):
                        ...
                        if not self.enemy.is_alive():
                            ...
                            self.state = "CAMP"
                            self.save_current_game() # Auto-save après combat
                        else:
