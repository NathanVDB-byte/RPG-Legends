...
POSSIBLE_CONSUMABLES = [
    ...
    {"name": "Peau de Pierre (+2)", "type": "def", "val": 2, "desc": "+2 Base Def", "cat": "consommable"}
]

POSSIBLE_EQUIPMENT = [
    # ARMES (+ATK)
    {"name": "Épée Rouillée", "cat": "equipment", "slot": "weapon", "atk": 5, "def": 0, "hp": 0, "desc": "+5 ATK"},
    {"name": "Hache de Guerre", "cat": "equipment", "slot": "weapon", "atk": 12, "def": -2, "hp": 0, "desc": "+12 ATK / -2 DEF"},
    # TÊTE (+DEF)
    ...
    # BAGUE (Bonus divers)
    {"name": "Anneau Vitalité", "cat": "equipment", "slot": "ring", "atk": 0, "def": 0, "hp": 40, "desc": "+40 HP"},
...
pygame.display.set_caption("Mini RPG - Fixed Inventory")
...
class Character:
    def __init__(self, name, hp, attack, defense, job_class="Guerrier"):
        self.name = name
        self.base_max_hp = hp # PV de base sans équipement
        self.hp = hp
    ...
    @property
    def max_hp(self):
        # Stats totales = Base + Somme des bonus d'équipement
    ...
    def equip_item(self, item_index):
        if 0 <= item_index < len(self.inventory):
            ...
            slot = item["slot"]
            # Bonus de PV de l'item qu'on va mettre
            new_hp_bonus = item.get("hp", 0)
            
            # Si on remplace un item, on enlève ses PV bonus
            old_item = self.equipment[slot]
            if old_item:
                old_hp_bonus = old_item.get("hp", 0)
                self.hp = max(1, self.hp - old_hp_bonus) # On baisse les HP actuels
                self.inventory.append(old_item) # Retour inv
            
            self.equipment[slot] = item
            self.inventory.pop(item_index)
            
            # On ajoute les PV bonus aux PV actuels (soin gratuit lors de l'équipement)
    ...
    def unequip_item(self, slot):
        ...
        if item:
            # On retire le bonus de PV des PV actuels
    ...
    def from_dict(cls, data):
        # Support pour anciennes sauvegardes (migration basique)
        base_hp = data.get("base_max_hp", data.get("max_hp", 100))
        base_atk = data.get("base_attack", data.get("attack", 15))
        base_def = data.get("base_defense", data.get("defense", 5))
        
        c = cls(data["name"], base_hp, base_atk, base_def, data.get("job_class", "Aventurier"))
        c.hp = data["hp"]
...
class Game:
    ...
    def refresh_inventory_ui(self):
        self.item_buttons = []
        if self.player:
            # Mode "Sac à Dos" : On affiche TOUT (Consommables + Equipements)
            if self.state == "INVENTORY":
                ...
                for i, item in enumerate(self.player.inventory):
                    ...
                    col_btn = PURPLE if item.get("cat") == "equipment" else BLUE
                    
                    btn = Button(item["name"], x, y, 220, 80, col_btn, data=(item, i))
                    self.item_buttons.append(btn)
            
            # Mode "Equipement" : On affiche SEULEMENT les équipements
    ...
    def run(self):
        while True:
            ...
            if self.state == "MENU":
                ...
                self.draw_text_centered("(Tout vos items)", FONT_SMALL, 90, LIGHT_GRAY)
                ...
                for slot_name, btn in self.equip_slots_buttons.items():
                    ...
                    if equipped_item:
                        btn.text = equipped_item["name"][:12] # Tronque si trop long
                        btn.color = GREEN
                    else:
                ...
                for event in events:
                    ...
                    # Clic sur un item de l'inventaire
                    for btn in self.item_buttons:
                         if btn.is_clicked(event, self.inv_scroll_y, check_bounds=True):
                            item, index = btn.data
