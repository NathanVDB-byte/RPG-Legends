...
pygame.display.set_caption("Mini RPG - Settings Update")
...
class Game:
    def __init__(self):
        self.state = "MENU"
        self.previous_state = "MENU" # Pour se souvenir d'où on vient
    ...
    def setup_ui(self):
        ...
        # --- NOUVEAU BOUTON SETTINGS (Petit, en haut à droite) ---
    ...
    def run(self):
        while True:
            ...
            for event in events:
                ...
                # Gestion du bouton Paramètres (Dispo presque partout sauf Menu Principal et Load)
            ...
            if self.state == "MENU":
                ...
                # Pas de bouton options ici, logique de base
                for event in events:
            ...
            # --- NOUVEL ETAT PARAMETRES ---
                ...
                for event in events:
                    ...
                    if self.btn_set_menu.is_clicked(event):
                        self.save_current_game() # Sauvegarde de sécurité
                        self.state = "MENU"
                ...
                screen.blit(FONT_TEXT.render(f"[PV] Sante : {p.hp}/{p.max_hp}", True, WHITE), (250, 260))
                screen.blit(FONT_TEXT.render(f"[ATK] Attaque : {p.attack_value}", True, WHITE), (250, 300))
                block_pct = min(60, p.defense_value * 2)
                screen.blit(FONT_TEXT.render(f"[DEF] Defense : {p.defense_value} ({block_pct}%)", True, WHITE), (250, 340))
                screen.blit(FONT_TEXT.render(f"[ETAGE] Etage : {p.floor}  |  [KILLS] : {p.kills}/10", True, ORANGE), (250, 380))
                screen.blit(FONT_TEXT.render(f"($) Or : {p.gold}", True, GOLD), (250, 420))
                ...
                self.btn_settings.check_hover(pos); self.btn_settings.draw(screen) # Opt
                ...
                if not self.item_buttons:
                    screen.blit(FONT_SMALL.render("Pas d'equipement...", True, GRAY), (50, 120 + self.inv_scroll_y))
