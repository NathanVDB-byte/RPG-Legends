...
POSSIBLE_CONSUMABLES = [
    {"name": "Potion Soin (15)", "type": "heal", "val": 15, "desc": "Rend 15 PV", "cat": "consommable"},
    {"name": "Grde Potion (30)", "type": "heal", "val": 30, "desc": "Rend 30 PV", "cat": "consommable"},
    {"name": "Elixir Force (+2)", "type": "atk", "val": 2, "desc": "+2 Atk (Perm)", "cat": "consommable"},
    {"name": "Peau de Pierre (+2)", "type": "def", "val": 2, "desc": "+2 Def (Perm)", "cat": "consommable"}
]

# NOUVEAU : LISTE DES ÉQUIPEMENTS
# cat: equipment, slot: head/chest/legs/feet/weapon/ring
POSSIBLE_EQUIPMENT = [
    # ARMES
    {"name": "Épée Rouillée", "cat": "equipment", "slot": "weapon", "atk": 5, "def": 0, "hp": 0, "desc": "Vieux mais coupant"},
    {"name": "Hache de Guerre", "cat": "equipment", "slot": "weapon", "atk": 12, "def": -2, "hp": 0, "desc": "Lourde et puissante"},
    {"name": "Dague Vicieuse", "cat": "equipment", "slot": "weapon", "atk": 8, "def": 0, "hp": 0, "desc": "Rapide"},
    # TÊTE
    {"name": "Casque de Cuir", "cat": "equipment", "slot": "head", "atk": 0, "def": 2, "hp": 10, "desc": "Protège un peu"},
    {"name": "Heaume d'Acier", "cat": "equipment", "slot": "head", "atk": 0, "def": 5, "hp": 0, "desc": "Solide"},
    # PLASTRON
    {"name": "Tunique en Tissu", "cat": "equipment", "slot": "chest", "atk": 0, "def": 1, "hp": 20, "desc": "Confortable"},
    {"name": "Cotte de Mailles", "cat": "equipment", "slot": "chest", "atk": 0, "def": 8, "hp": 0, "desc": "Lourde protection"},
    # JAMBES
    {"name": "Jambières Cuir", "cat": "equipment", "slot": "legs", "atk": 0, "def": 3, "hp": 5, "desc": "Souple"},
    # PIEDS
    {"name": "Bottes de Marche", "cat": "equipment", "slot": "feet", "atk": 0, "def": 1, "hp": 5, "desc": "Pour voyager loin"},
    {"name": "Bottes Cloutées", "cat": "equipment", "slot": "feet", "atk": 1, "def": 2, "hp": 0, "desc": "Écrase les orteils"},
    # BAGUE
    {"name": "Anneau de Vie", "cat": "equipment", "slot": "ring", "atk": 0, "def": 0, "hp": 30, "desc": "Une aura vitale"},
    {"name": "Anneau de Force", "cat": "equipment", "slot": "ring", "atk": 4, "def": 0, "hp": 0, "desc": "Puissance magique"}
]

BOSS_LOOT = [
    {"name": "Grde Potion (30)", "type": "heal", "val": 30, "desc": "Rend 30 PV", "cat": "consommable"},
    {"name": "Épée Légendaire", "cat": "equipment", "slot": "weapon", "atk": 20, "def": 5, "hp": 0, "desc": "Lame des Anciens"}
]
...
pygame.display.set_caption("Mini RPG - Équipement & Builds")
...
class Character:
    def __init__(self, name, hp, attack, defense, job_class="Guerrier"):
        ...
        self.inventory = [] # Liste d'objets (dicts)
        
        # NOUVEAU : Slots d'équipement
        self.equipment = {
            "head": None,
            "chest": None,
            "legs": None,
    ...
    # --- GETTERS DYNAMIQUES POUR LES STATS TOTALES ---
    @property
    def max_hp(self):
    ...
    def take_damage(self, dmg):
        ...
        block_chance = min(60, total_def * 2) # Cap à 60%
        is_blocked = False
    ...
    def heal(self, amount):
        old = self.hp
        # On utilise la propriété max_hp qui inclut les bonus
        self.hp = min(self.max_hp, self.hp + amount)
        return f"Soin: +{self.hp - old} PV"

    def equip_item(self, item_index):
        """Déplace un item de l'inventaire vers le slot d'équipement."""
        if 0 <= item_index < len(self.inventory):
            item = self.inventory[item_index]
            if item.get("cat") != "equipment": return "Pas équipable"
            
            slot = item["slot"]
            # Si déjà un item, on le déséquipe d'abord (swap)
    ...
    def unequip_item(self, slot):
        """Retire l'item du slot et le remet dans l'inventaire."""
        if self.equipment[slot]:
    ...
    def from_dict(cls, data):
        c = cls(data["name"], data["base_max_hp"], data["base_attack"], data["base_defense"], data.get("job_class", "Aventurier"))
        c.hp = data["hp"]
...
class Game:
    ...
    def setup_ui(self):
        ...
        self.btn_rest = Button("Se Reposer", 270, 450, 220, 50, GREEN)
        self.btn_inventory = Button("Sac à Dos", 270, 380, 150, 50, PURPLE)
        
        # NOUVEAU BOUTON ÉQUIPEMENT (Petit carré)
        ...
        # Slots d'équipement (pour le menu EQUIP_MENU)
        # Positions fixes pour Tête, Plastron, Jambes, Pieds, Arme, Bague
    ...
    def refresh_inventory_ui(self):
        ...
        if self.player:
            ...
            # Mode Inventaire "classique" (Pour l'écran INVENTORY)
            if self.state == "INVENTORY":
                for i, item in enumerate(self.player.inventory):
                    row = i // cols; col = i % cols
                    x = start_x + (col * col_width); y = start_y + (row * row_height)
                    btn = Button(item["name"], x, y, 220, 80, PURPLE, data=(item, i))
                    self.item_buttons.append(btn)
            # Mode "Liste à côté" (Pour l'écran EQUIP_MENU)
            elif self.state == "EQUIP_MENU":
                start_x = 50; start_y = 120; cols = 1
                for i, item in enumerate(self.player.inventory):
                    # On affiche SEULEMENT les équipements dans ce menu
                    if item.get("cat") == "equipment":
                        row = len(self.item_buttons) # Stack vertical
    ...
    def run(self):
        while True:
            ...
            for event in events:
                ...
                if event.type == pygame.MOUSEWHEEL and self.state == "INVENTORY":
                    self.inv_scroll_y += event.y * 30
                    if self.inv_scroll_y > 0: self.inv_scroll_y = 0

                if self.state in ["CAMP", "STATS_VIEW", "INVENTORY", "EQUIP_MENU"]:
            ...
            if self.state == "MENU":
                ...
                # Affichage stats avec les bonus d'équipement
                ...
                self.draw_text_centered("CONSO & ITEMS", FONT_TITLE, 50, PURPLE)
                ...
                for btn in self.item_buttons:
                    ...
                    if btn.hover:
                         item, _ = btn.data
                         desc_txt = FONT_SMALL.render(item["desc"], True, GOLD)
                ...
                # Partie Gauche: Inventaire (Items filtrés)
                self.draw_text_centered("Inventaire (Equipements)", FONT_SMALL, 100, LIGHT_GRAY)
                clip_rect = pygame.Rect(0, 100, 250, 420) # Colonne gauche
                screen.set_clip(clip_rect)
                for btn in self.item_buttons:
                    btn.check_hover(pos, self.inv_scroll_y); btn.draw(screen, self.inv_scroll_y)
                screen.set_clip(None)

                # Partie Droite: Silhouette
                for slot_name, btn in self.equip_slots_buttons.items():
                    # Si un item est équipé, on change le texte et la couleur
                    equipped_item = self.player.equipment.get(slot_name)
                    if equipped_item:
                        btn.text = equipped_item["name"]
                        btn.color = GREEN
                    else:
                ...
                for event in events:
                    ...
                    # Clic sur un item de l'inventaire -> Équiper
                    for btn in self.item_buttons:
                         if btn.is_clicked(event, self.inv_scroll_y, check_bounds=True):
                            item, index = btn.data
                            msg = self.player.equip_item(index)
                            self.add_log(msg)
                            self.refresh_inventory_ui() # Mise à jour listes
                            self.save_current_game()
                            break
                    
                    # Clic sur un slot -> Déséquiper
                    for slot_name, btn in self.equip_slots_buttons.items():
                        if btn.is_clicked(event):
                ...
                else: self.btn_rest.text = "Se Reposer"; self.btn_rest.disabled = False
                
                for btn in [self.btn_explore, self.btn_rest, self.btn_inventory, self.btn_save, self.btn_equip_menu]:
                    btn.check_hover(pos); btn.draw(screen)
                self.draw_logs()
                ...
                for event in events:
                    ...
                    if self.btn_equip_menu.is_clicked(event):
                        self.refresh_inventory_ui(); self.state = "EQUIP_MENU"
                    ...
                    if self.btn_attack.is_clicked(event):
                        ...
                        if not self.enemy.is_alive():
                            ...
                            if self.enemy.is_boss:
                                ...
                                # LOOT (Consommable ou Equipement)
                                if random.random() < 0.35: # 35% Chance de loot global
                                    if random.random() < 0.7: # 70% chance que ce soit une potion
                                        loot_item = random.choice(POSSIBLE_CONSUMABLES).copy()
                                    else: # 30% chance d'équipement
