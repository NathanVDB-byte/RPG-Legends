...
pygame.display.set_caption("Mini RPG - Inventaire Interactif")
...
class Button:
    ...
    def draw(self, surface):
        # Effet visuel au survol (plus clair)
    ...
    def refresh_inventory_ui(self):
        ...
        if self.player:
            ...
            for i, item in enumerate(self.player.inventory):
                ...
                # Le bouton contient l'item entier dans 'data'
        ...
        while True:
            ...
            # =================== INVENTAIRE (INTERACTIF) ===================
                ...
                for event in events:
                    ...
                    # Logique d'utilisation des items
                    for btn in self.item_buttons:
                        if btn.is_clicked(event):
                            item = btn.data
                            ...
                            # Effet selon le type
                            if item["type"] == "heal":
                                msg = self.player.heal(item["val"])
                                self.add_log(f"Utilisé: {item['name']}")
                                self.add_log(msg)
                            elif item["type"] == "atk":
                            ...
                            self.refresh_inventory_ui() # On redessine les boutons sans l'item
                            self.save_current_game() # Sauvegarde persistence
                            break # On sort de la boucle pour éviter des erreurs d'index
                ...
                for event in events:
                    if self.btn_attack.is_clicked(event):
                        ...
                        if not self.enemy.is_alive():
                            # LOOT !
                            loot_item = random.choice(POSSIBLE_LOOT).copy() # .copy() IMPORTANT
