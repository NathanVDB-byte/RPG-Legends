...
POSSIBLE_CONSUMABLES = [
    ...
    {"name": "Elixir Force (+2)", "type": "atk", "val": 2, "desc": "+2 Base Atk", "cat": "consommable"},
    {"name": "Peau Pierre (+2)", "type": "def", "val": 2, "desc": "+2 Base Def", "cat": "consommable"}
]

POSSIBLE_EQUIPMENT = [
    {"name": "Épée Rouillée", "cat": "equipment", "slot": "weapon", "atk": 5, "def": 0, "hp": 0, "desc": "+5 ATK"},
    {"name": "Hache Guerre", "cat": "equipment", "slot": "weapon", "atk": 12, "def": -2, "hp": 0, "desc": "+12 ATK/-2 DEF"},
    {"name": "Casque Cuir", "cat": "equipment", "slot": "head", "atk": 0, "def": 3, "hp": 0, "desc": "+3 DEF"},
    {"name": "Heaume Acier", "cat": "equipment", "slot": "head", "atk": 0, "def": 6, "hp": 0, "desc": "+6 DEF"},
    {"name": "Tunique Tissu", "cat": "equipment", "slot": "chest", "atk": 0, "def": 2, "hp": 20, "desc": "+2 DEF/+20 HP"},
    {"name": "Cotte Mailles", "cat": "equipment", "slot": "chest", "atk": 0, "def": 10, "hp": 10, "desc": "+10 DEF/+10 HP"},
    {"name": "Jambières Cuir", "cat": "equipment", "slot": "legs", "atk": 0, "def": 4, "hp": 0, "desc": "+4 DEF"},
    {"name": "Bottes Marche", "cat": "equipment", "slot": "feet", "atk": 0, "def": 2, "hp": 0, "desc": "+2 DEF"},
    {"name": "Anneau Vie", "cat": "equipment", "slot": "ring", "atk": 0, "def": 0, "hp": 40, "desc": "+40 HP"},
    {"name": "Anneau Rage", "cat": "equipment", "slot": "ring", "atk": 5, "def": 0, "hp": 0, "desc": "+5 ATK"}
]
...
# Flag RESIZABLE pour permettre le switch fullscreen plus facilement
screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT))
pygame.display.set_caption("Mini RPG - Fullscreen & Fixes")
...
FONT_TEXT = pygame.font.SysFont("Arial", 22) # Font un peu plus petite pour aider
...
class Character:
    ...
    def from_dict(cls, data):
        c = cls(
            data["name"], 
            data.get("base_max_hp", data.get("max_hp", 100)), # Compatibilité
        ...
        # --- MIGRATION DE DONNÉES (Pour fixer les potions invisibles) ---
        for item in c.inventory:
            if "cat" not in item:
                # Si l'objet n'a pas de catégorie, on devine
                if item.get("slot"): # Si ça a un slot, c'est un équipement
                    item["cat"] = "equipment"
                else: # Sinon c'est une potion
                    item["cat"] = "consommable"
...
class Button:
    ...
    def draw(self, surface, offset_y=0):
        ...
        # --- GESTION DU TEXTE QUI DEPASSE ---
        display_text = self.text
        # On réduit le texte tant qu'il est trop large pour le bouton (avec une marge de 10px)
        while FONT_TEXT.size(display_text)[0] > self.rect.width - 10 and len(display_text) > 0:
            display_text = display_text[:-1]
        
        # Si on a tronqué, on pourrait ajouter "...", mais pour faire simple on coupe net
        # Une version plus propre retirerait 3 chars et ajouterait "..."
        if len(display_text) < len(self.text):
...
class Game:
    def __init__(self):
        ...
        self.inv_scroll_y = 0
        self.is_fullscreen = False # Etat fullscreen
        self.setup_ui()
    ...
    def setup_ui(self):
        ...
        # NOUVEAU : Bouton Fullscreen (Petit carré en haut à droite)
        self.btn_fullscreen = Button("⛶", 750, 20, 30, 30, GRAY)
        ...
        if self.player:
            if self.state == "INVENTORY":
                ...
                for i, item in enumerate(self.player.inventory):
                    ...
                    # Couleur : Bleu pour potion, Violet pour équipement
                    col_btn = BLUE if item.get("cat") == "consommable" else PURPLE
    ...
    def run(self):
        while True:
            ...
            for event in events:
                ...
                if event.type == pygame.MOUSEWHEEL and self.state in ["INVENTORY", "EQUIP_MENU"]:
            ...
            if self.state == "MENU":
                ...
                self.btn_fullscreen.check_hover(pos); self.btn_fullscreen.draw(screen) # Fullscreen au menu aussi
                for event in events:
                ...
                self.draw_text_centered("(Bleu: Potions / Violet: Items)", FONT_SMALL, 90, LIGHT_GRAY)
                ...
                for btn in self.item_buttons:
                    ...
                    if btn.hover:
                        item, _ = btn.data
                        desc = item.get("desc", "")
                        screen.blit(FONT_SMALL.render(desc, True, GOLD), (btn.rect.x, btn.rect.y + 35 + self.inv_scroll_y))
                ...
                for slot_name, btn in self.equip_slots_buttons.items():
                    ...
                    if equipped_item:
                        # Tronquage manuel pour le slot aussi
