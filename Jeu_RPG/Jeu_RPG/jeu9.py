...
POSSIBLE_LOOT = [
    ...
    {"name": "Peau de Pierre (+5)", "type": "def", "val": 5, "desc": "+5 Défense perm."},  # NOUVEAU
    {"name": "Acier Liquide (+10)", "type": "def", "val": 10, "desc": "+10 Défense perm."} # NOUVEAU
]
...
pygame.display.set_caption("Mini RPG - Défense Avancée")
...
class Character:
    ...
    def take_damage(self, dmg):
        """
        Calcule les dégâts subis avec réduction fixe ET chance de parade.
        Retourne (dégâts_réels, est_paré)
        """
        # 1. Réduction fixe (Armure)
        reduced_dmg = max(0, dmg - (self.defense_value // 2)) # On adoucit la réduction fixe pour valoriser la parade
        
        # 2. Chance de Parade (Block Chance)
        # Chaque point de défense donne 2% de chance de parer (Capé à 50%)
        ...
        if roll < block_chance:
            is_blocked = True
            reduced_dmg = reduced_dmg // 2 # Réduit de moitié si paré
    ...
    def attack_target(self, target):
        ...
        # Appel de la nouvelle méthode take_damage
        actual, blocked = target.take_damage(raw_dmg)
        
        crit_str = " (CRITIQUE!)" if crit > 1 else ""
        block_str = " [🛡️ PARÉ]" if blocked else ""
        
        return f"{self.name} attaque {target.name}{crit_str}{block_str}: -{actual} PV"
    ...
    def run(self):
        while True:
            ...
            # =================== STATS ===================
                ...
                # Affiche la défense + le pourcentage de parade
                block_pct = min(50, p.defense_value * 2)
                def_txt = f"🛡️ Défense : {p.defense_value} (Parade: {block_pct}%)"
                screen.blit(FONT_TEXT.render(def_txt, True, WHITE), (250, 340))
                ...
                for event in events:
                    ...
                    if self.btn_delete.is_clicked(event):
                        self.delete_current_save()
                        self.player = None; self.refresh_save_list()
                ...
                for event in events:
                    ...
                    for btn in self.item_buttons:
                        if btn.is_clicked(event):
                            ...
                            elif item["type"] == "def": # NOUVEAU TYPE
                                self.player.defense_value += item["val"]
                                self.add_log(f"Armure renforcée ! (+{item['val']} DEF)")
            ...
            # =================== CAMP ===================
                ...
                if time_left > 0:
                    self.btn_rest.text = f"Repos ({time_left//1000}s)"; self.btn_rest.disabled = True
                else:
                    self.btn_rest.text = "Se Reposer (+10PV)"; self.btn_rest.disabled = False
                ...
                for event in events:
                    if self.btn_attack.is_clicked(event):
                        ...
                        if not self.enemy.is_alive():
                            ...
                            # Contre-attaque ennemie (Utilise aussi la nouvelle parade)
                            # Mais ici c'est le joueur qui prend les dégâts
                    ...
                    if self.btn_flee.is_clicked(event):
                        if random.random() < 0.5:
                            ...
                            dmg, blocked = self.player.take_damage(self.enemy.attack_value)
                            block_msg = " [BLOQUÉ]" if blocked else ""
                            self.add_log(f"Coups reçus : -{dmg} PV{block_msg}")
                            if not self.player.is_alive(): self.state = "MENU"
