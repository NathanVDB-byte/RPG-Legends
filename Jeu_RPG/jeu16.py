...
pygame.display.set_caption("Mini RPG - Final")
...
class Game:
    ...
    def run(self):
        while True:
            ...
            current_time = pygame.time.get_ticks() # ICI, c'est corrigé !
            ...
            if self.state == "MENU":
                ...
                for btn in self.item_buttons:
                    ...
                    # Description sous le bouton (Mode Sac à Dos)
                    if btn.hover:
                         item, _ = btn.data
                ...
                for btn in self.item_buttons:
                    btn.check_hover(pos, self.inv_scroll_y); btn.draw(screen, self.inv_scroll_y)
                    # Description À DROITE du bouton (Mode Équipement) pour éviter le chevauchement
                    if btn.hover:
                        item, _ = btn.data
                        desc = item.get("desc", "")
                        # Affichage à droite du bouton
                ...
                for event in events:
                    ...
                    # CORRECTION ICI: On change d'état PUIS on rafraichit
